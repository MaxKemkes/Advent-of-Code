...
function checkAdjacecncy(
    ...
    for (let i = 0; i < nums.length; i++){
        adj[i] = nums[i].filter(e => {
            const above = specialChars[Math.max(0, i -1)].some(char =>  (Math.max(e[1] -1,0)) <= char[1] && char[1] <= (e[1] + e[0].length ))
            const inline = specialChars[i].some(char =>  (e[1] -1) === char[1] || char[1] === (e[1] + e[0].length))
