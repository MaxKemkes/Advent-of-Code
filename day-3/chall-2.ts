...
function calcGearFactor(
    ...
    for (let i = 0; i < specialChars.length; i++){
        adj[i] = specialChars[i].map(e => {
            ...
            if (i > 0)[
                above = nums[Math.max(0, i -1)].filter(n => !(n[1] + n[0].length -1 < e[1]-1 || n[1] > e[1]+1) )
            ]
            // console.log(i)
            // console.log(nums[i])
            console.log(e)
            const inline = nums[i].filter(n =>  (e[1] -1) === (n[1] + n[0].length -1) || e[1] +1  === (n[1]))
            if (i < specialChars.length - 1){
                below = nums[Math.min(maxLen, i + 1)].filter(
					(n) => !(n[1] + n[0].length -1 < e[1] - 1 || n[1] > e[1] + 1)
				);
